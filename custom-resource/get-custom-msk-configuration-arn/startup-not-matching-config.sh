...
aws cloudformation create-stack --stack-name MSK-GetCustomConfiguration-Stack --template-body file://get-custom-msk-config.yaml --parameters ParameterKey="KafkaVersion",ParameterValue="2.4.1" ParameterKey="S3BucketName",ParameterValue="dev-valentine" ParameterKey="S3ObjectKeyName",ParameterValue="DataToShare/custom-config-info-not-matching" --capabilities CAPABILITY_IAM
...
then
    echo " -- Got ARN of the created custom MSK configruation:"
