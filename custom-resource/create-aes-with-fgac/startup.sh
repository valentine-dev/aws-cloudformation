...
aws cloudformation create-stack --stack-name Valentine-Test-AES-FGAC-Stack --template-body file://aes-fgac.yaml --parameters ParameterKey="DomainName",ParameterValue="valentine-test-aes-fgac" --capabilities CAPABILITY_IAM
echo " -- Start creating stack Valentine-Test-AES-FGAC-Stack ..."
...
if aws cloudformation describe-stacks --stack-name Valentine-Test-AES-FGAC-Stack | sed -n 's/.*\"StackStatus\": //p' | tr -d ',' | grep -q 'CREATE_COMPLETE';
then
